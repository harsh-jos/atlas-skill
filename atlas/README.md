...
Atlas Go keeps a map of the territory, lights up the parts you've walked, and always has a
...
- **"Teach me consistent hashing"** → it researches what it needs to, writes a single
  right-sized HTML lesson into `pages/`, marks the topic learned in `atlas-go.json`, wires the
...
**The repo is the memory — not a model's session memory.** `atlas-go.json` is the single source
...
The full rationale lives in `.claude/skills/atlas-go/references/pedagogy.md`.

## Layout

```
atlas-go/
  atlas-go.json              the map + memory: domains, topics, status, prereqs, links
  index.html              the home map — GENERATED, never hand-edited
  assets/atlas-go.css        the design system (one file; retheme here)
  pages/                  one HTML lesson per learned topic
  scripts/build_index.py  rebuilds index.html from atlas-go.json
  .claude/skills/atlas-go/
    ...
    references/
      ...
      atlas-go-schema.md     atlas-go.json schema + the next-topic (BFS) logic
```

## Using it

1. **Open the repo in Claude Code.** The skill in `.claude/skills/atlas-go/` is picked up
   automatically.
...
4. **Watch the map fill in.** Each lesson updates `atlas-go.json` and regenerates `index.html`.
...
It reads `atlas-go.json` and writes `index.html`. The skill runs this for you after each lesson;
you'd only run it by hand after editing `atlas-go.json` directly.

## Seeding your own topics

`atlas-go.json` ships with ~95 topics across 12 domains as a starting territory. Add your own by
appending objects to the `topics` array (see `references/atlas-go-schema.md` for the shape):
...
One CSS file, `assets/atlas-go.css`, deliberately avoiding the usual generated-UI defaults. It
...
the top of `atlas-go.css` and the whole system follows. The signature element is the **knowledge
